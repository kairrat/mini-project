const currencies = [{
  ...
  const api = (currency,currency2) => `
    ${apiBase}convert?q=${currency}_${currency2}&compact=ultra&apiKey=5ef6e14ef0d61cd65d93`;

    const toast = (msg) => {
        const toastr = document.querySelector('.message');
        if(!toastr) return;
  ...
  const startApp = () =>{
      ...
      draw.addEventListener("click", ()=>{
        document.getElementsByClassName("conversion")[0].style.display="flex";
        convert(event);
      });
  }
  startApp(); 
